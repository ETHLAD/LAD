...
fn get_kernel_entry_point_address(st: &mut SystemTable<Boot>) -> u64{
    ...
    if let Elf::Elf64(ref e) = elf {
        for p in e.program_header_iter() {
            ...
            kernel_start_addr = core::cmp::min(kernel_start_addr, p.vaddr());
            kernel_end_addr = core::cmp::max(kernel_end_addr, p.vaddr() + p.memsz());
        }
    }
    ...
    if let Elf::Elf64(ref e) = elf {
        for p in e.program_header_iter() {
            ...
            //let seg = p.segment();
            let seg = p.content();
            let dst_addr = p.vaddr();
            let seg_len = p.filesz();
    ...
    if let Elf::Elf64(ref e) = elf {
        entry_point_address = e.entry_point();
    }
